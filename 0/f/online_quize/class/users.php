...
class users
{
	...
	public function login($email,$pass)                  // funtion for login(called in login_submit.php)
	{
		...
		$query=$this->conn->query("select email,password from student_soes where email='$email' and password='$pass'");
	...
	{
		$query=$this->conn->query("select * from student_soes where email='$email'");
	...
	{
		$query=$this->conn->query("select * from category_f");
