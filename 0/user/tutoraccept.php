...
<h6 style="margin:0 auto" class="bg-dark m-auto text-warning"><?=$s;?></h6>
       ...
       <div class="row">
         <div style="border: 2px solid black; border-radious:10px" class="col-md-2 border">
                ...
                <div class="col mt-2">
                    <a href="index.php"><button type="button" class="btn btn-outline-info mt-2 w-100">All Users</button></a>
                    <a href="friend.php"><button type="button" class="btn btn-outline-danger mt-2 w-100">Friends</button></a>
         ...
         <div style="border: 2px solid black; border-radious:10px" class="col border-dark">
            <div class="row">
            ...
            if(isset($_SESSION['unique_id'])){
                  ...
                  $frq="SELECT * FROM users WHERE unique_id='$from_id'";
                  $frqr=mysqli_query($con,$frq);
                  $rowf=mysqli_fetch_assoc($frqr);
                ...
                //   }
                  ...
                  $queryuser2="SELECT * FROM tutor_request WHERE  status='Pending' AND tutor_id='$from_id'";
                  ...
                  while($row2 =mysqli_fetch_assoc($queryuresult2)):
                    ...
                    <?php
                    $stdq="SELECT * FROM users WHERE unique_id='$row2[std_id]'";
                    $stdqr=mysqli_query($con,$stdq);
                    $rowstd=mysqli_fetch_assoc($stdqr);
                    ?>
                    <div style="border: 1px solid black; border-radious:10px" class="col-md-6 border-info">
                                
                    <div class="notifi-item">
                      <?php if($rowstd['img']!=''){
                        echo '<img style="padding-top:0px" class="d-inline" src="../image/'.$rowstd['img'].'" alt="img">';
                      }else{
                      ...
                      <?php if($rowstd['name']!=''){
                        echo "<h4>".$rowstd['name']."</h4>";
                      }else{
                        echo "<h4>Username</h4>";
                      } ?>

                     <?php if($rowstd['district']==''){
                        $row['district'] ="userDistrict";
                      } ?>
                      <?php if($rowstd['upazila']==''){
                        $row['upazila'] ="Upazila";
                      } ?>
                      <?php if($rowstd['unions']==''){
                        $row['unions'] ="Union";
                      } ?>
                       

                        <p>@<?= $rowstd['district'].', '.$rowstd['upazila'].', '.$rowstd['unions']?></p>
                        <span class="text-center text-danger"><a style="text-decoration:none; color;red" class="text-uppercase text-danger" href="user.php?unique_id=<?=$row2['from_id']?>">PROFILE DETAILS</a></span>
                      </div>
                      <div class="text text-center m-auto">
                      <button type="button" class="btn btn-outline-info d-block w-100">ID: <?=$row2['post_id']?></button>
                      <form action="tutorsubmit.php" method="POST" enctype="multipart/form-data">
                      <input type="hidden" name="updaterequest" value="<?=$row2['id']?>">
                      <input type="hidden" name="fromid" value="<?=$from_id?>">
                      <input type="hidden" name="toid" value="<?=$row2['std_id']?>">
                      <input type="hidden" name="postid" value="<?=$row2['post_id']?>">
                    <?php
                      if(isset($_SESSION['unique_id'])){
                       echo '<button type="submit" name="acceptstd" class="btn btn-outline-primary d-block w-100">Accept</button>';
                      }
                      ?>
