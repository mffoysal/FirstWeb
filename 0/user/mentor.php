...
$qq = "SELECT * FROM tutor WHERE unique_id!='' AND status!='Hide' ";
$record=mysqli_num_rows(mysqli_query($con,$qq));
  ...
  $record=mysqli_num_rows(mysqli_query($con,$qq));
    ...
    $pagi=ceil($record/$per_page);
          ...
          if(!empty($searchkey)){    
              $query = "SELECT * FROM tutor WHERE class LIKE '%$searchkey%' AND status!='Hide' OR phone LIKE '%$searchkey%' AND status!='Hide' OR sub LIKE '%$searchkey%' AND status!='Hide' OR address LIKE '%$searchkey%' AND status!='Hide' OR unique_id LIKE '%$searchkey%' AND status!='Hide' OR post_id LIKE '%$searchkey%' AND status!='Hide' limit $start, $per_page";
              ...
              // }
              $query = "SELECT * FROM tutor WHERE unique_id!='' AND status!='Hide' limit $start, $per_page";
              $searchkey = "";
          }
         ...
         <div style="border: 2px solid black; border-radius:10px" class="col border-light">
            <div class="row">
                  ...
                  <?php
                    ...
                    $value = user();
                    while($rowt =mysqli_fetch_assoc($value)):
                    ?>

                        <?php
                            $mq="SELECT * FROM users WHERE unique_id='$rowt[unique_id]'";
                        ...
                        ?>
                                            ...
                                            <div class="col-md-6 mt-2 mb-2">
                                              <div class="card bg-transparent border-info text-white h-100">
                                                  ...
                                                  <img style="height: 300px;" src="<?php 
                                                  if($rowt['img']!=''){
                                                    echo '../image/'.$rowt['img'];
                                                  }else if($row['img']!=''){
                                                    echo '../image/'.$row['img'];
                                                  ...
                                                  ?>
                                                  <!-- farhad/chat.php?user_id=787579835 -->
                                                  <a href="../farhad/chat.php?user_id=<?=$row['unique_id']?>"><button type="button" class="btn btn-outline-success w-100">TuTor</button></a>
                                                  <?php
                                                  if(isset($_SESSION['unique_id'])){

                                                    $msquery="SELECT * FROM tutor_request WHERE std_id='$_SESSION[unique_id]' AND post_id='$rowt[post_id]' AND status='Pending'";
                                                    $msqueryr=mysqli_query($con,$msquery);
                                                    $countms=mysqli_num_rows($msqueryr);
                                                    $msquery2="SELECT * FROM tutor_request WHERE std_id='$_SESSION[unique_id]' AND post_id='$rowt[post_id]' AND status='Confirm'";
                                                    $msqueryr2=mysqli_query($con,$msquery2);
                                                    $countms2=mysqli_num_rows($msqueryr2);
                                                    $msquery3="SELECT * FROM tutor_request WHERE std_id!='$_SESSION[unique_id]' AND post_id='$rowt[post_id]' AND status='Confirm'";
                                                    $msqueryr3=mysqli_query($con,$msquery3);
                                                    $countms3=mysqli_num_rows($msqueryr3);
                                                    

                                                    if($countms>0){
                                                        echo '<span  class="btn btn-outline-info w-100" >Pending for You</span>';    
                                                    }else if($countms2>0){
                                                        echo '<a href="activated.php?unique_id='.$rowt['unique_id'].'"><span  class="btn btn-outline-info w-100" >Activated</span></a>';
                                                    }else if($countms3>0){
                                                        echo '<a href="user.php?unique_id='.$rowt['unique_id'].'"><span  class="btn btn-outline-info w-100" >Tutor Confirmed</span></a>';
                                                    }else {
                                                        echo '<button type="submit" name="addmentor" class="btn btn-outline-info w-100" >ADD TuTor</button>';
                                                    }
                                                  ...
                                                  <div  class="card-footer bg-light text-dark">&copy; <?=$row['name']?> <span class="bg-warning">T.P.ID: <?=$rowt['post_id']?></span>

                                                  <button type="button" class="btn btn-outline-success d-inline w-100 position-relative" data-bs-toggle="modal" data-bs-target="#details<?=$row['unique_id']?>">
                                                  <?php
                                                  ...
                                                  <a href="javascript:void(0)"><button onclick="love_update('<?=$rowt['id']?>')" class="btn btn-outline-info mt-1"><span class="bi bi-heart-fill"> (<span id="love_loop_<?=$rowt['id']?>"><?=$rowt['love_count']?></span>)</span ></button></a>
                                                  <a href="javascript:void(0)"><button onclick="like_update('<?=$rowt['id']?>')" class="btn btn-outline-success mt-1"><span class="bi bi-hand-thumbs-up-fill"> (<span id="like_loop_<?=$rowt['id']?>"><?=$rowt['like_count']?></span>)</span ></button></a>
                                                  <a href="javascript:void(0)"><button onclick="dislike_update('<?=$rowt['id']?>')" class="btn btn-outline-warning mt-1"><span class="bi bi-hand-thumbs-down-fill"> (<span id="dislike_loop_<?=$rowt['id']?>"><?=$rowt['dislike_count']?></span>)</span></button></a>
