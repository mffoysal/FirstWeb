<?php
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['demail'] && $_SESSION['key']=='suryapinky') 
    {
      ...
      $result = mysqli_query($con,"DELETE FROM quiz_user WHERE email='$demail' ") or die('Error');
  ...
  if(isset($_SESSION['key']))
  {
    ...
    {
      $eid=@$_GET['eid'];
      $result = mysqli_query($con,"SELECT * FROM quizquestions WHERE eid='$eid' ") or die('Error');
      while($row = mysqli_fetch_array($result)) 
      {
        $qid = $row['qid'];
        $r1 = mysqli_query($con,"DELETE FROM options WHERE qid='$qid'") or die('Error');
        $r2 = mysqli_query($con,"DELETE FROM qanswer WHERE qid='$qid' ") or die('Error');
      }
      $r3 = mysqli_query($con,"DELETE FROM quizquestions WHERE eid='$eid' ") or die('Error');
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['q']== 'addqns' && $_SESSION['key']=='suryapinky') 
    {
      ...
      for($i=1;$i<=$n;$i++)
      {
        $qid=uniqid();
        $qns=$_POST['qns'.$i];
        $q3=mysqli_query($con,"INSERT INTO quizquestions VALUES  ('$eid','$qid','$qns' , '$ch' , '$i')");
        $oaid=uniqid();
        $obid=uniqid();
        $ocid=uniqid();
        $odid=uniqid();
        ...
        $qans=mysqli_query($con,"INSERT INTO qanswer VALUES  ('$qid','$ansid')");
      }
  ...
  if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) 
  {
    ...
    $ans=$_POST['ans'];
    $qid=@$_GET['qid'];
    $q=mysqli_query($con,"SELECT * FROM qanswer WHERE qid='$qid' " );
