...
foreach ($question_id as $key => $value) {
	...
	if(isset($option_id[$key])){
		...
		$is_right = $con->query("SELECT * FROM question_opt where id = '".$option_id[$key]."' ")->fetch_array()['is_right'];
	}
	$data .= ', is_right = "'.$is_right.'" ';
	$insert = $con->query("INSERT INTO qanswers set ".$data);
...
$total = count($question_id) * $qpoints;
	...
	$insert2 = $con->query("INSERT INTO history set ".$data);
	if($insert2)
